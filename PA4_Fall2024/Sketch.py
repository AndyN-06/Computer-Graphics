...
set base rendering mode to light
vectors to hold scenes and light statuses
added bools to see what lights are on
changed arrow keys to switch between scenes
changes scene switch
added light toggle
added asd keys for light modes and 123 keys for light toggle
'''
import os
import math
import time
...
class Sketch(CanvasBase):
    ...
    # variable to see whether we use normal rendering or not
    normalRenderingOn = False 

    # Variables for scene and lights
    currentSceneIndex = 1
    lights_status = []
    original_light_colors = []
    ...
    def __init__(self, parent):
        ...
        # initialize lighting
        self.ambientEnabled = False
        self.diffuseEnabled = False
        self.specularEnabled = False

    def toggleLight(self, light_index):
        if 0 <= light_index < len(self.lights_status):
            self.lights_status[light_index] = not self.lights_status[light_index]
            light = self.scene.lights[light_index]
            light.setColor(np.array((0, 0, 0, 1))) if not self.lights_status[light_index] else light.setColor(light.color)
            self.shaderProg.setLight(light_index, light)
            print(f"Light {light_index + 1} {'enabled' if self.lights_status[light_index] else 'disabled'}")
    ...
    def switchScene(self, sceneIndex):
        if sceneIndex == 1:
            self.scene = SceneOne(self.shaderProg)
        elif sceneIndex == 2:
            self.scene = SceneTwo(self.shaderProg)
        elif sceneIndex == 3:
            self.scene = SceneThree(self.shaderProg)
        else:
        ...
        # Initialize lights_status based on the number of lights in the new scene
        self.lights_status = [True] * len(self.scene.lights)
        # Store original light colors
        self.original_light_colors = [light.color.copy() for light in self.scene.lights]
        # self.scene = scene
        # self.topLevelComponent.clear()
        # self.topLevelComponent.addChild(self.scene)
        # self.topLevelComponent.initialize()
    ...
    def toggleLight(self, light_index):
        if 0 <= light_index < len(self.lights_status):
            self.lights_status[light_index] = not self.lights_status[light_index]
            light = self.scene.lights[light_index]
            if self.lights_status[light_index]:
                # Restore original color
                light.setColor(self.original_light_colors[light_index])
            else:
                # Set color to black to turn off the light
                light.setColor(np.array([0.0, 0.0, 0.0, 1.0]))
            self.shaderProg.setLight(light_index, light)
            print(f"Light {light_index + 1} {'enabled' if self.lights_status[light_index] else 'disabled'}")
        else:
            print(f"No light with index {light_index + 1}")
    ...
    def InitGL(self):
        ...
        self.switchScene(self.currentSceneIndex)


        # Set the initial rendering mode
        self.shaderProg.setFragmentShaderRouting("lighting")
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if keycode in [wx.WXK_RETURN]:
            print("Enter key pressed: Updating canvas")
            self.update()
        if keycode == wx.WXK_LEFT:
            # Switch to previous scene
            self.currentSceneIndex = (self.currentSceneIndex - 2) % 3 + 1  # Move to previous scene
            print(f"Switching to Scene {self.currentSceneIndex}")
            self.switchScene(self.currentSceneIndex)
        if keycode == wx.WXK_RIGHT:
            # Switch to next scene
            self.currentSceneIndex = self.currentSceneIndex % 3 + 1  # Move to next scene
            print(f"Switching to Scene {self.currentSceneIndex}")
            self.switchScene(self.currentSceneIndex)
        if keycode in [wx.WXK_UP]:
            print("Up arrow key pressed: Scrolling up")
            self.Interrupt_Scroll(1)
            self.update()
        if keycode in [wx.WXK_DOWN]:
            print("Down arrow key pressed: Scrolling down")
            self.Interrupt_Scroll(-1)
            self.update()
        ...
        if chr(keycode) in "pP":
            print(f"P key pressed: {'Pausing' if not self.pauseScene else 'Resuming'} scene")
            self.pauseScene = not self.pauseScene
        if chr(keycode) in "iI":
            self.ImageModeOn = not self.ImageModeOn
            print(f"I key pressed: {'Enabling' if self.ImageModeOn else 'Disabling'} Image Mode")
            self.shaderProg.setBool("imageFlag", self.ImageModeOn)

        if chr(keycode) in "nN":  # Add this block to handle 'n' key
            self.normalRenderingOn = not self.normalRenderingOn
            if self.normalRenderingOn:
                print("N key pressed: Enabling normal rendering mode")
                self.shaderProg.setFragmentShaderRouting("normal")
            else:
                print("N key pressed: Disabling normal rendering mode")
                # Set back to your default rendering mode, e.g., "lighting"
                self.shaderProg.setFragmentShaderRouting("lighting")
            self.update()

        # todo 4.2
        if chr(keycode) in "aA":  # Toggle ambient lighting
            self.ambientEnabled = not self.ambientEnabled
            self.shaderProg.setBool("ambientEnabled", self.ambientEnabled)
            print(f"Ambient lighting {'enabled' if self.ambientEnabled else 'disabled'}")
        if chr(keycode) in "dD":  # Toggle diffuse lighting
            self.diffuseEnabled = not self.diffuseEnabled
            self.shaderProg.setBool("diffuseEnabled", self.diffuseEnabled)
            print(f"Diffuse lighting {'enabled' if self.diffuseEnabled else 'disabled'}")
        if chr(keycode) in "sS":  # Toggle specular lighting
            self.specularEnabled = not self.specularEnabled
            self.shaderProg.setBool("specularEnabled", self.specularEnabled)
            print(f"Specular lighting {'enabled' if self.specularEnabled else 'disabled'}")

        # TODO 5.3 is at here
        if ord('1') <= keycode <= ord('9'):
            light_index = keycode - ord('1')
            self.toggleLight(light_index)
